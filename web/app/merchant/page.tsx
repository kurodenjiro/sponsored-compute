...
export default function MerchantPage() {
  ...
  /**
   * Dùng lại đúng loader của app (.route-loader trong globals.css) thay vì tự
   * vẽ: trang này nạp ledger phía client nên nó là màn hình đầu tiên người xem
   * thấy, và trước đây nó là serif chữ thường trong khi mọi route khác hiện
   * wordmark — nhìn như lạc sang site khác giữa lúc demo.
   */
  if (!data && !error) {
    return <main className="route-loader" role="status" aria-label="Loading merchant ledger">
      <div><b>sponsored<span>compute</span></b><i /><small>loading merchant ledger</small></div>
    </main>;
  }

  if (!data) return <main className="route-loader route-loader-error" role="alert">
    <div>
      <b>sponsored<span>compute</span></b>
      <small>merchant ledger unavailable</small>
      <p>{error}</p>
      <button onClick={() => window.location.reload()}>retry</button>
    </div>
    <style jsx>{`
      .route-loader-error p{margin:0;max-width:460px;text-align:center;color:#9ba697;font:12px/1.6 ui-monospace,monospace}
      .route-loader-error button{margin-top:8px;padding:10px 16px;border:1px solid #caff4b;background:#caff4b;color:#10140f;font:800 10px ui-monospace,monospace;letter-spacing:.08em;text-transform:uppercase;cursor:pointer}
    `}</style>
  </main>;
