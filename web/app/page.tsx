...
const prompt = 'Claude, turn this repo into a working SaaS onboarding flow. Use its sponsored grant for the database call.';
...
export default function Home() {
  ...
  const state = !submitted ? 'user is typing' : phase >= 6 ? 'settled on-chain' : phase >= 5 ? 'paying with grant' : phase >= 4 ? 'policy checkpoint' : phase >= 2 ? 'calling MCP tools' : phase >= 1 ? 'Claude is working' : 'prompt sent';
    ...
    <section id="flow" className="flow">
      ...
      <div className="terminal">
        ...
        <div className="conversation" ref={threadRef}>
          {!submitted && <div className="composer-demo"><label>YOU</label><div><p>{typedPrompt}<i aria-hidden="true" /></p><button type="button" disabled>{typedPrompt.length === prompt.length ? 'SEND' : `${typedPrompt.length}/${prompt.length}`}</button></div></div>}
