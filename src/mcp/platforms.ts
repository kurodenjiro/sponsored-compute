...
export const PLATFORMS: Platform[] = [
  ...
  { id: 'supadb', name: 'SupaDB', category: 'database', sponsored: true,
    ...
    note: 'Postgres quản lý, realtime, auth kèm sẵn' },
  { id: 'neonlite', name: 'NeonLite', category: 'database', sponsored: true,
    ...
    note: 'Postgres serverless, tách compute/storage' },
  { id: 'postgres-self', name: 'PostgreSQL (tự host)', category: 'database', sponsored: false,
    campaignId: null, grantSgd: null, fitScore: 92, x402: false,
    note: 'Miễn phí, không khoá nhà cung cấp. Bạn tự vận hành.' },
  { id: 'sqlite', name: 'SQLite', category: 'database', sponsored: false,
    campaignId: null, grantSgd: null, fitScore: 74, x402: false,
    note: 'Không cần server. Hợp dự án nhỏ / edge.' },
  ...
  { id: 'sentrywatch', name: 'SentryWatch', category: 'monitoring', sponsored: true,
    ...
    note: 'Theo dõi lỗi + trace' },
  { id: 'otel-self', name: 'OpenTelemetry (tự host)', category: 'monitoring', sponsored: false,
    campaignId: null, grantSgd: null, fitScore: 79, x402: false,
    note: 'Chuẩn mở, không khoá nhà cung cấp.' },
];
...
export function renderPlatforms(category?: string): string {
  ...
  if (rows.length === 0) return `Không có nền tảng nào trong nhóm "${category}".`;

  const nS = rows.filter((r) => r.sponsored).length;
  const nU = rows.length - nS;

  const lines = rows.map((p, i) => {
    const tag = p.sponsored ? `[CÓ TÀI TRỢ · ${p.grantSgd} SGD]` : '[không tài trợ]';
    return `${i + 1}. ${p.name} ${tag}\n   phù hợp ${p.fitScore}/100 · ${p.x402 ? 'x402' : 'không x402'}\n   ${p.note}`;
  });

  return [
    `${nS} có tài trợ · ${nU} không tài trợ — xếp theo độ phù hợp kỹ thuật, KHÔNG theo tài trợ.`,
    '',
    ...lines,
    '',
    'Bạn chọn nền tảng nào? (bạn quyết định, không phải agent)',
  ].join('\n');
}
